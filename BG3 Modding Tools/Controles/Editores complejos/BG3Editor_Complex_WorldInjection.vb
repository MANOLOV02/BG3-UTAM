...
Public Class BG3Editor_Complex_WorldInjection
    Public Objeto As BG3_Obj_Stats_Class
    Public Modsource As BG3_Pak_SourceOfResource_Class
    ...
    End Sub
    Public Sub Read_Data(ByRef Objeto As BG3_Obj_Stats_Class, Modsource As BG3_Pak_SourceOfResource_Class)
        Me.Objeto = Objeto
        Me.Modsource = Modsource
        Dim nam As String = "I_" + Objeto.Name
        ListBox1.Items.Clear()
        Dim tabesta As Boolean
        For Each tabl In FuncionesHelpers.GameEngine.ProcessedTTables.ElementValues.Where(Function(pf) pf.HasItem(nam) = True)
            tabesta = False
                ...
                If Subt.Source.Pak_Or_Folder = Modsource.Pak_Or_Folder Then
                    For Each lis In Subt.Lista
                        If lis.Item.Contains(nam, StringComparison.OrdinalIgnoreCase) Then
                            ...
                            Dim Ite As New BG3_Custom_ComboboxItem With {.Value = lis2, .Text = tabl.Name}
    ...
    Public Sub Write_Data()
        ...
        For Each it In ListBox1.Items
            For Each tabl In FuncionesHelpers.GameEngine.ProcessedTTables.ElementValues.Where(Function(pf) pf.Name = it.text)
                tabesta = False
                tabl.CanMerge = True
                If tabl.Subtables.Where(Function(pf) pf.Source.Pak_Or_Folder = Modsource.Pak_Or_Folder AndAlso pf.HasItem(nam)).Any = False Then
                    ...
                    subt.Lista.Add(New BG3_Obj_TreasureTable_TableItem_Class(lis2.Item, lis2.Conditions))
                    tabl.Subtables.Add(subt)
                End If
                For Each Subt In tabl.Subtables.Where(Function(pf) pf.Source.Pak_Or_Folder = Modsource.Pak_Or_Folder AndAlso pf.HasItem(nam))
                    Dim lis2 As BG3_Obj_TreasureTable_TableItem_Class = it.value
                    For Each lis In Subt.Lista
                        If lis.Item.Contains(nam, StringComparison.OrdinalIgnoreCase) Then
    ...
    Private Sub DropTT(obj As BG3_Obj_TreasureTable_Class)
        ...
        For x = 0 To ListBox1.Items.Count - 1
            If ListBox1.Items(x).text = obj.Name Then idx = x : Exit For
        Next
        If idx = -1 Then
            Dim lis As New BG3_Obj_TreasureTable_TableItem_Class("I_" + Objeto.Name, "," + NumericUpDown1.Value.ToString + ",0,0,0,0,0,0,0")
            Dim Ite As New BG3_Custom_ComboboxItem With {.Value = lis, .Text = obj.Name}
            ListBox1.Items.Add(Ite)
        End If
        For x = 0 To ListBox1.Items.Count - 1
            If ListBox1.Items(x).text = obj.Name Then ListBox1.SelectedIndex = x : Exit For
        Next
    End Sub
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        If ListBox1.SelectedIndex <> -1 Then
            ...
            Dim nam As String = "I_" + Objeto.Name
            For Each tabl In FuncionesHelpers.GameEngine.ProcessedTTables.ElementValues.Where(Function(pf) pf.Name = it.text)
                If tabl.Subtables.Where(Function(pf) pf.Source.Pak_Or_Folder = Modsource.Pak_Or_Folder AndAlso pf.HasItem(nam)).Any = True Then
                    For Each Subt In tabl.Subtables.Where(Function(pf) pf.Source.Pak_Or_Folder = Modsource.Pak_Or_Folder AndAlso pf.HasItem(nam)).ToList
                        Dim lis2 As BG3_Obj_TreasureTable_TableItem_Class = it.value
