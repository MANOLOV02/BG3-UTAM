...
Public Class BG3Selector_Stat
    Inherits BG3Selector_Generic_Code(Of BG3_Obj_Stats_Class)
    ...
    Public Overrides Sub SplitGroup(sender As Object, e As EventArgs)
        ...
        Dim nuevos_grupoas As New SortedList(Of String, BG3_Custom_TreeNode_Linked_Class(Of BG3_Obj_Stats_Class))
        For x = actualnod.Nodes.Count - 1 To 0 Step -1
            Dim nod = actualnod.Nodes(x)
            Dim selectedtmp As BG3_Obj_Stats_Class = CType(CType(nod, Object).objeto, BG3_Obj_Stats_Class)
            Dim suffix As String = "_" + Attr_or_data + "_"
            Select Case queleo
                Case "Data"
                    If Attr_or_data <> "Using" Then
                        suffix += selectedtmp.Get_Data_Or_Inherited_or_Empty(Attr_or_data)
                    Else
                        suffix += selectedtmp.Using
                    End If
                Case "Attribute"
                    suffix += selectedtmp.ReadAttribute_Or_Inhterithed_Or_Empty(Attr_or_data)
                Case "Custom"
                    RaiseEvent FillSplit(Attr_or_data, CType(CType(selectedtmp, Object), BG3_Obj_Stats_Class), suffix)
                Case Else
                    Debugger.Break()
            End Select
    ...
    Public Event FillSplit(ByVal Attr_or_data As String, ByRef selectedtmp As BG3_Obj_Stats_Class, ByRef suffix As String)
