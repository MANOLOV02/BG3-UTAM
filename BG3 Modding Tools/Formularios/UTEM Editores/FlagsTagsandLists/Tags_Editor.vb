...
Public Class Tags_Editor
    ...
    Protected Overridable Property NameNod As String = "Tags"
    ...
    Protected Overridable ReadOnly Property UtamType As BG3_Enum_UTAM_Type = BG3_Enum_UTAM_Type.Tag
    ...
    Protected Sub Initialize(ByRef MdiParent As Main, Source As BG3_Pak_SourceOfResource_Class)
        ...
        HandledAttributes.Add("Name")
        HandledAttributes.Add("DisplayDescription")
        HandledAttributes.Add("_OriginalFileVersion_")

        HandledNodes.Add("Tags\Categories")
        HandledNodes.Add("Tags\Properties")
    ...
    Private Sub Habilita_Edicion_Botones(Edicion As Boolean)
        ...
        GroupBoxCategories.Enabled = Edicion
        GroupBoxProperties.Enabled = Edicion
    ...
    Private Sub PictureBox3_DragEnter(sender As Object, e As DragEventArgs) Handles PictureBox3.DragEnter
        If GroupBoxBasicTemplates.Enabled = True Then BG3Editor_FlagsandTag_Icon1.Control_DragEnter(sender, e)
    End Sub
    Private Sub PictureBox3_DragDrop(sender As Object, e As DragEventArgs) Handles PictureBox3.DragDrop
        If GroupBoxBasicTemplates.Enabled = True Then BG3Editor_FlagsandTag_Icon1.Control_DragDrop(sender, e)
    End Sub
    Private Sub Capture_Icon_Changed(sender As Object) Handles BG3Editor_FlagsandTag_Icon1.Inside_Text_Changed
        If Not IsNothing(SelectedTmp) Then
            Dim ic As BG3_Obj_IconUV_Class = Nothing
            PictureBox3.Image = Nothing
            If FuncionesHelpers.GameEngine.ProcessedIcons.TryGetValue(BG3Editor_FlagsandTag_Icon1.TextBox1.Text, ic) Then
    ...
    Private Sub Capture_Names_Changed(sender As Object) Handles BG3Selector_FlagsandTags1.Change_Selected, BG3Editor_Complex_Localization1.LocaTextChanged
        If Not IsNothing(SelectedTmp) Then
            LabelInfoName.Text = "Name: " + BG3Editor_Complex_Localization1.Get_Localization(BG3Editor_FlagsandTag_DisplayName1, SelectedTmp, FuncionesHelpers.GameEngine.Settings.SelectedLocalization)
            LabelInfoDescription.Text = "Description: " + BG3Editor_Complex_Localization1.Get_Localization(BG3Editor_FlagsandTag_DisplayDescription1, SelectedTmp, FuncionesHelpers.GameEngine.Settings.SelectedLocalization)
    ...
    Private Sub Capture_Clone(obj As BG3_Obj_FlagsAndTags_Class, tipo As BG3Cloner.Clonetype) Handles BG3Selector_FlagsandTags1.Clone_tag
        ...
        SelectedTmp = FuncionesHelpers.GameEngine.ProcessedFlagsAndTags.Manage_Overrides(New BG3_Obj_FlagsAndTags_Class(Clone_Nuevonod, ActiveModSource, BG3_Enum_FlagsandTagsType.Tags))
    ...
    Private Sub Process_delete()
        ...
        End If
        If FuncionesHelpers.GameEngine.ProcessedLocalizationList.ContainsKey(BG3Editor_FlagsandTag_DisplayDescription1.Get_Utam_Handle(SelectedTmp)) = True Then
            FuncionesHelpers.GameEngine.ProcessedLocalizationList.Remove(BG3Editor_FlagsandTag_DisplayDescription1.Get_Utam_Handle(SelectedTmp))
        End If
    ...
    Private Sub Create_Initial(ByRef nuevonod As LSLib.LS.Node)
        ...
        BG3Editor_FlagsAndTag_Name1.Create_Attribute(nuevonod, "NEW_TAG")
        BG3Editor_FlagsandTag_DisplayName1.Create_Attribute(nuevonod, Funciones.NewGUID(True) + ";1")
        BG3Editor_FlagsandTag_DisplayDescription1.Create_Attribute(nuevonod, Funciones.NewGUID(True) + ";1")
    ...
    Protected Overridable Sub Create_Initial_Specific(ByRef nuevonod As LSLib.LS.Node)
        Dim cat As New LSLib.LS.Node With {.Name = "Categories"}
        Dim nnod As New LSLib.LS.Node With {.Name = "Category", .Parent = cat}
        nnod.Attributes.TryAdd("Name", New LSLib.LS.NodeAttribute(AttributeType.LSString) With {.Value = "Code"})
        cat.AppendChild(nnod)
        nuevonod.AppendChild(cat)
    End Sub
    ...
    Protected Overridable Sub Process_Selection_Change_specific()
        ...
        ButtonDeletePropery.Enabled = False
        If Not IsNothing(SelectedTmp) Then
            Dim value As LSLib.LS.Node

            Dim values As List(Of LSLib.LS.Node) = Nothing

            If SelectedTmp.NodeLSLIB.Children.TryGetValue("Categories", values) Then
                If values.First.Children.Count > 0 Then
                    For Each value In values.First.Children("Category")

                        Dim value2 As NodeAttribute = Nothing
                        If value.Attributes.TryGetValue("Name", value2) Then
                            ListBox1.Items.Add(value2.AsString(Funciones.Guid_to_string))
                        End If
                    Next
                End If
            End If
            ...
            If SelectedTmp.NodeLSLIB.Children.TryGetValue("Properties", values2) Then
                If values2.First.Children.Count > 0 Then
                    For Each value In values2.First.Children("Property")
                        Dim value2 As NodeAttribute = Nothing
                        If value.Attributes.TryGetValue("Name", value2) Then
                            ListBox2.Items.Add(value2.AsString(Funciones.Guid_to_string))
                        End If
                    Next
                End If
            End If
    ...
    Private Sub Process_Selection_Change()
        If Not IsNothing(SelectedTmp) Then
            ...
            BG3Editor_FlagsAndTag_Description1.Read(SelectedTmp)
            BG3Editor_FlagsandTag_DisplayName1.Read(SelectedTmp)
            BG3Editor_FlagsandTag_DisplayDescription1.Read(SelectedTmp)
            BG3Editor_FlagsandTag_Icon1.Read(SelectedTmp)
        ...
        Else
            ...
            BG3Editor_FlagsAndTag_Description1.Clear()
            BG3Editor_FlagsandTag_DisplayName1.Clear()
            BG3Editor_FlagsandTag_DisplayDescription1.Clear()
            BG3Editor_FlagsandTag_Icon1.Clear()
    ...
    Protected Overridable Sub Process_Save_Edits_Specifics()
        Dim cat As New LSLib.LS.Node With {.Name = "Categories"}
        Dim prop As New LSLib.LS.Node With {.Name = "Properties"}
        SelectedTmp.NodeLSLIB.Children.Remove("Categories")
        SelectedTmp.NodeLSLIB.Children.Remove("Properties")

        For Each it In ListBox1.Items
            Dim nnod As New LSLib.LS.Node With {.Name = "Category", .Parent = cat}
            nnod.Attributes.TryAdd("Name", New LSLib.LS.NodeAttribute(AttributeType.LSString) With {.Value = it.ToString})
            cat.AppendChild(nnod)
        Next

        For Each it In ListBox2.Items
            Dim nnod As New LSLib.LS.Node With {.Name = "Property", .Parent = prop}
            nnod.Attributes.TryAdd("Name", New LSLib.LS.NodeAttribute(AttributeType.LSString) With {.Value = it.ToString})
            prop.AppendChild(nnod)
        Next
        SelectedTmp.NodeLSLIB.AppendChild(prop)
        SelectedTmp.NodeLSLIB.AppendChild(cat)
    End Sub
    ...
    Private Sub Process_Save_Edits()
        ...
        BG3Editor_FlagsAndTag_Description1.Write(SelectedTmp)
        BG3Editor_FlagsandTag_DisplayName1.Write(SelectedTmp)
        BG3Editor_FlagsandTag_DisplayDescription1.Write(SelectedTmp)
        BG3Editor_FlagsandTag_Icon1.Write(SelectedTmp)
...
#Enable Warning CA1861 ' Evitar matrices constantes como argumentos
        ...
        For Each nod As BG3_Custom_TreeNode_Linked_Class(Of BG3_Obj_FlagsAndTags_Class) In nods.Nodes
            ...
            If obj IsNot Template Then
                ...
                For Each stat In statsList
                    Select Case stat
                        Case "Icon"
                            BG3Editor_FlagsandTag_Icon1.Read(Template)
                            BG3Editor_FlagsandTag_Icon1.Write(obj)
                        Case Else
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles ButtonDeleteCategory.Click
        If ListBox1.SelectedIndex <> -1 Then
            ListBox1.Items.RemoveAt(ListBox1.SelectedIndex)
        End If
    End Sub

    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles ButtonDeletePropery.Click
    ...
    Private Sub Button4_Click(sender As Object, e As EventArgs) Handles Button4.Click
        If TextBox2.Text <> "" Then
            If ListBox2.Items.IndexOf(TextBox2.Text) = -1 Then
    ...
    Private Sub ListBox1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ListBox1.SelectedIndexChanged
        ButtonDeleteCategory.Enabled = Not ListBox1.SelectedIndex = -1
    End Sub

    Private Sub ListBox2_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ListBox2.SelectedIndexChanged
        ButtonDeletePropery.Enabled = Not ListBox2.SelectedIndex = -1
    End Sub
End Class