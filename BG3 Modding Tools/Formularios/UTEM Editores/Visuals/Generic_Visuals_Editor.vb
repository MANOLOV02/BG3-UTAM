...
Public Class Generic_Visuals_Editor
    Sub New()
        MyBase.New
        ' Esta llamada es exigida por el diseñador.
        InitializeComponent()
        ' Agregue cualquier inicialización después de la llamada a InitializeComponent().
        Flickering.EnableDoubleBuffering(BG3Selector_Visuals1.TreeView1)
    End Sub

    Public HandledAttributes As New List(Of String)
    Public HandledNodes As New List(Of String)
    Protected Overridable Property Visualtype As BG3_Enum_VisualBank_Type = BG3_Enum_VisualBank_Type.TextureBank
    Protected Overridable Property NameNod As String = "Resource"
    Public Property ActiveModSource As BG3_Pak_SourceOfResource_Class
    Protected Overridable Property SelectedTmp As BG3_Obj_VisualBank_Class
    Protected Overridable ReadOnly Property Prefix As String = "UTAM_XXXX_"
    Protected Overridable ReadOnly Property UtamType As BG3_Enum_UTAM_Type = BG3_Enum_UTAM_Type.Texture
    ...
    Private Sub Form_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        BG3Selector_Visuals1.Selection = UtamType
        BG3Selector_Visuals1.Load_Templates(FuncionesHelpers.GameEngine.UtamVisuals)
    ...
    Private Sub Explore_Node_DoubleClicked(nod As Object)
        If BG3Selector_Visuals1.IsEditing OrElse BG3Selector_Visuals1.Isclonning_or_transfering Then Exit Sub
        Select Case nod.GetType
            Case GetType(BG3_Custom_TreeNode_Linked_Class(Of BG3_Obj_Template_Class))
                If Not IsNothing(CType(nod, BG3_Custom_TreeNode_Linked_Class(Of BG3_Obj_Template_Class)).Objeto) Then
                    ...
                    Dim find = BG3Selector_Visuals1.TreeView1.Nodes.Find(obj.MapKey, True)
                    If find.Length > 0 Then
                        BG3Selector_Visuals1.TreeView1.SelectedNode = find.First
                    End If
                End If
            Case GetType(BG3_Custom_TreeNode_Linked_Class(Of BG3_Obj_Stats_Class))
                If Not IsNothing(CType(nod, BG3_Custom_TreeNode_Linked_Class(Of BG3_Obj_Stats_Class)).Objeto) Then
                    ...
                    If Not IsNothing(obj.AssociatedTemplate) Then
                        Dim find = BG3Selector_Visuals1.TreeView1.Nodes.Find(obj.AssociatedTemplate.MapKey, True)
                        If find.Length > 0 Then
                            BG3Selector_Visuals1.TreeView1.SelectedNode = find.First
                        End If
                    End If
    ...
    Protected Sub Initialize(ByRef MdiParent As Main, Source As BG3_Pak_SourceOfResource_Class)
        ...
        BG3Selector_Visuals1.BG3Cloner1.RadioButtonInherit.Enabled = False
        BG3Selector_Visuals1.BG3Cloner1.RadioButtonClone.Checked = True
        BG3Selector_Visuals1.ByInheritingToolStripMenuItem.Enabled = False
        BG3Selector_Visuals1.WorldInjectionToolStripMenuItem.Visible = False
        BG3Selector_Visuals1.TreasureTablesToolStripMenuItem.Visible = False
        ...
        'BG3Editor_Complex_Localization1.Link_Controls({BG3Editor_FlagsandTag_DisplayName1, BG3Editor_FlagsandTag_DisplayDescription1})
    ...
    Private Sub Habilita_Edicion_Botones(Edicion As Boolean)
        ...
        BG3Selector_Visuals1.BG3Cloner1.Enabled = Not Edicion
    ...
    Protected Overridable Sub Capture_Clone_specific(obj As BG3_Obj_VisualBank_Class, tipo As BG3Cloner.Clonetype)
    ...
    Private Sub Capture_Clone(obj As BG3_Obj_VisualBank_Class, tipo As BG3Cloner.Clonetype) Handles BG3Selector_Visuals1.Clone_Visual
        Clone_Nuevonod = New LSLib.LS.Node With {.Name = NameNod}
        Template_guid = Funciones.NewGUID(False)
        BG3Selector_Visuals1.Habilita_Edicion_Botones(True)
        ' template Clone
        Select Case tipo
            ...
            Case BG3Cloner.Clonetype.Clone
                ...
                Create_Initial(Clone_Nuevonod)
                Editor_Generic_GenericAttribute.Replace_Attribute_Generic(Clone_Nuevonod, "UTAM_Group", BG3Selector_Visuals1.Current_Group, AttributeType.FixedString)
                Capture_Clone_specific(obj, tipo)
            Case BG3Cloner.Clonetype.Override
                ...
                End If
                If obj.NodeLSLIB.TryGetOrEmpty("DisplayDescription") <> "" AndAlso obj.NodeLSLIB.TryGetOrEmpty("DisplayName") <> obj.NodeLSLIB.TryGetOrEmpty("DisplayDescription") Then
                    Editor_Generic_GenericAttribute.Replace_Attribute_Generic(Clone_Nuevonod, "UTAM_h2", obj.NodeLSLIB.TryGetOrEmpty("DisplayDescription"), AttributeType.TranslatedString)
                End If
                Capture_Clone_specific(obj, tipo)
            Case Else
                Debugger.Break()
        ...
        SelectedTmp = FuncionesHelpers.GameEngine.ProcessedVisualBanksList.Manage_Overrides(New BG3_Obj_VisualBank_Class(Clone_Nuevonod, ActiveModSource, VisualType))
        ...
        Select Case tipo
            ...
            Case BG3Cloner.Clonetype.Clone, BG3Cloner.Clonetype.Inherit, BG3Cloner.Clonetype.Override
                ...
                CLone_Loca(obj.NodeLSLIB.TryGetOrEmpty("DisplayDescription"), SelectedTmp.NodeLSLIB.TryGetOrEmpty("UTAM_h2"))
            Case Else
                Debugger.Break()
        End Select
    ...
    Protected Sub CLone_Loca(OldMapkey As String, UtamHandle As String)
        ...
        Next
    End Sub
    Private Sub Capture_AddNew(Group As String) Handles BG3Selector_Visuals1.Add_New_Click
        Capture_Clone(Nothing, BG3Cloner.Clonetype.None)
        BG3Selector_Visuals1.Habilita_Edicion_Botones(True)
        Habilita_Edicion_Botones(True)
    End Sub
    Private Sub Capture_Edit(template As BG3_Obj_VisualBank_Class) Handles BG3Selector_Visuals1.Edit_Click
        If IsNothing(template) Then Exit Sub
        Process_Edit()
    End Sub
    Private Sub Capture_Cancel() Handles BG3Selector_Visuals1.Cancel_Click
        Process_Cancel()
    End Sub
    Private Sub Capture_Save() Handles BG3Selector_Visuals1.Save_Click
        Process_Save_Edits()
    End Sub
    Private Sub Capture_Delete() Handles BG3Selector_Visuals1.Delete_Click
        Process_delete()
    End Sub
    ...
    Private Sub Process_delete()
        ...
        FuncionesHelpers.GameEngine.UtamVisuals.Remove(SelectedTmp)
        FuncionesHelpers.GameEngine.ProcessedVisualBanksList.Remove(SelectedTmp)
        Process_Delete_Specifics()
        BG3Selector_Visuals1.Delete_Ended()
    End Sub
    Private Sub Capture_Selection_Change(Template As BG3_Obj_VisualBank_Class) Handles BG3Selector_Visuals1.Change_Selected
    ...
    End Sub
    Private Sub Select_Objects(Template As BG3_Obj_VisualBank_Class)
    ...
    Private Sub Add_UTAM_Attribute(ByRef nuevonod As LSLib.LS.Node)
        ...
        Editor_Generic_GenericAttribute.Create_Attribute_Generic(nuevonod, "UTAM_Group", BG3Selector_Visuals1.Current_Group, AttributeType.FixedString)
    ...
    Private Sub Process_Selection_Change()
        If Not IsNothing(SelectedTmp) Then
            ...
            BG3Selector_Visuals1.BG3Editor_Template_UtamGroup1.Read(SelectedTmp)
            Process_Selection_Change_specific()
            BG3Editor_Complex_Advanced_Attributes1.Read(SelectedTmp)
        Else
            BG3Selector_Visuals1.BG3Editor_Template_UtamGroup1.Clear()
            Process_Selection_Change_specific()
    ...
    Private Sub Process_Save_Edits()
        BG3Selector_Visuals1.BG3Editor_Template_UtamGroup1.Write(SelectedTmp)
    ...
    Private Sub Process_Save_Final()
        BG3Selector_Visuals1.Edit_Ended(SelectedTmp)
    End Sub
    ...
    Private Sub Create_Stat_Transfers()
        ...
        'New ToolStripMenuItem("Template|Icon|False|Attribute", Nothing, AddressOf BG3Selector_Visuals1.TransferSibligsClick) With {.Tag = {"Icon"}}
        '}
        ...
        For Each cat In Cats
            Dim indx = BG3Selector_Visuals1.PropertiesToolStripMenuItem.DropDown.Items.Add(New ToolStripMenuItem(cat))
            Dim indx2 = BG3Selector_Visuals1.SplitGroupButton.DropDown.Items.Add(New ToolStripMenuItem(cat))
            Dim indx3 = BG3Selector_Visuals1.PropertiesToolStripMenuItem1.DropDown.Items.Add(New ToolStripMenuItem(cat))
            For Each it In lista.Where(Function(pf) pf.Text.Split("|")(0) = cat).OrderBy(Function(pq) pq.Text.Split("|")(1)).Select(Function(pf) pf)
                ...
                If splitstr(2) = "True" Then
                    Dim splitx = New ToolStripMenuItem(splitstr(1), Nothing, AddressOf BG3Selector_Visuals1.SplitGroup) With {.Tag = CType(it.Tag, String()).Select(Function(pf) splitstr(3) + "|" + pf).ToArray}
                    CType(BG3Selector_Visuals1.SplitGroupButton.DropDown.Items(indx2), ToolStripMenuItem).DropDown.Items.Add(splitx)
                End If

                it.Text = splitstr(1)
                CType(BG3Selector_Visuals1.PropertiesToolStripMenuItem.DropDown.Items(indx), ToolStripMenuItem).DropDown.Items.Add(it)
                Dim Toall = New ToolStripMenuItem(it.Text, Nothing, AddressOf BG3Selector_Visuals1.TransferAllClick) With {.Tag = it.Tag}
                CType(BG3Selector_Visuals1.PropertiesToolStripMenuItem1.DropDown.Items(indx3), ToolStripMenuItem).DropDown.Items.Add(Toall)
            Next
        Next
    End Sub
    ...
    End Sub
    Protected Overridable Sub Transfer_stats_specifics(Template As BG3_Obj_VisualBank_Class, statsList() As String)
    ...
    Private Sub Transfer_stats(Template As BG3_Obj_VisualBank_Class, statsList() As String, nods As BG3_Custom_TreeNode_Linked_Class(Of BG3_Obj_VisualBank_Class)) Handles BG3Selector_Visuals1.Transfer_Stats
        Transfer_SaveOriginal()
        For Each nod As BG3_Custom_TreeNode_Linked_Class(Of BG3_Obj_VisualBank_Class) In nods.Nodes
            Dim obj As BG3_Obj_VisualBank_Class = nod.Objeto
            ...
            End If
        Next
        BG3Selector_Visuals1.Edit_Ended(Template)
    End Sub
    Private Sub Transfer_Handles(Template As BG3_Obj_VisualBank_Class, cual As Integer, nods As BG3_Custom_TreeNode_Linked_Class(Of BG3_Obj_VisualBank_Class)) Handles BG3Selector_Visuals1.Transfer_Localization
    ...
    Private Sub Generic_Editor_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
        If BG3Selector_Visuals1.IsEditing = True Then
            MsgBox("Save or cancel changes before closing", vbOKOnly, "Changes not saved")
            e.Cancel = True
        End If
    End Sub

    Private Sub FillSplit(ByVal Attr_or_data As String, ByRef selectedtmp As BG3_Obj_VisualBank_Class, ByRef suffix As String) Handles BG3Selector_Visuals1.FillSplit
    ...
    End Sub
    Protected Overridable Sub FillSplit_specific(ByVal Attr_or_data As String, ByRef selectedtmp As BG3_Obj_VisualBank_Class, ByRef suffix As String)
